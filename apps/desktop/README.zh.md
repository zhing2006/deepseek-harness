...
标准“编辑”菜单为当前获得焦点的窗口提供复制、粘贴、剪切、全选、撤销和重做快捷键，包括插件管理器的包输入框和可选中的错误文本。
...
打包应用运行编译后的 JavaScript 和预生成的 Typert 元数据，不编译 TypeScript 插件。源码级调试导航和编辑器声明仍可从开发包中获取。[复制规则测试](tests/runtime-file-policy.spec.ts)覆盖排除项和保留资源；`prepare:dsh` 在 Host smoke 和最终清单验证之前，使用内置 Node 针对目标平台的生产依赖执行[产物 smoke](tests/fixtures/runtime-payload-smoke.mjs)。
