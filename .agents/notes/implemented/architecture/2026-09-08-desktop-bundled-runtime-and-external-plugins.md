...
Package-specific exclusions remove Domino tests, Koffi's Windows import library, and non-target node-pty prebuilds and debug symbols. The policy retains native executable dependencies, node-pty's ConPTY source distribution, licenses, and unrecognized assets; broad `src`, `test`, `.ts`, or `.map` exclusions could remove executable code or runtime data. Copy tests preserve sentinel assets and seal the filtered inventory; the bundled-Node [payload smoke](../../../../apps/desktop/tests/fixtures/runtime-payload-smoke.mjs) verifies PTY output, FFI, image conversion, HTML parsing, and POSIX file-lock acquisition and contention through [node-addon-system](2026-09-07-prebuilt-system-primitives.md). Native smoke checks follow the target's production dependencies: Windows uses Koffi and does not load the POSIX flock addon. Requiring an absent dependency would reject a valid runtime; native loading or operation failures for applicable dependencies remain fatal. Runtime preparation still verifies every retained byte and boots the complete Host with an external plugin.
