...
包专用排除项包括 Domino 测试、Koffi 的 Windows 导入库，以及非目标平台的 node-pty 预构建文件和调试符号。规则保留原生可执行依赖、node-pty 的 ConPTY 源分发内容、许可证和未知资源；宽泛排除 `src`、`test`、`.ts` 或 `.map` 可能移除可执行代码或运行时数据。复制测试保留哨兵资源并封存过滤后的清单；内置 Node 的[产物 smoke](../../../../apps/desktop/tests/fixtures/runtime-payload-smoke.mjs)验证 PTY 输出、FFI、图像转换、HTML 解析，以及通过 [node-addon-system](2026-09-07-prebuilt-system-primitives.zh.md)执行的 POSIX 文件锁获取与争用。原生 smoke 检查遵循目标平台的生产依赖：Windows 使用 Koffi，不加载 POSIX flock 插件。要求不存在的依赖会拒绝有效运行时；适用依赖的原生加载或操作失败仍会终止打包。运行时准备仍会验证每个保留字节，并携带外部插件启动完整 Host。
